...
public class ExchangeUtils {
  ...
  /**
   * The configured masker, for the URI redaction path.
   *
   * <p>Endpoint URIs are redacted by {@link UriSanitizer} rather than here, because they are not
   * read through this class - the topology builds them from the route model and the tracers take
   * them from the event. They must still honour the same {@code camelbee.masked-keys}, so the
   * masker is shared rather than duplicated.
   *
   * @return the configured masker, never null.
   */
