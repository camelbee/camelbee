...
class ExchangeSendingEventTracerNewTest {
  ...
  @Test
  void initializeRouteStack_scansRoutesAndBackfillsPreviousMessageRouteId() {
    Exchange exchange = new DefaultExchange(camelContext);

    // Non-empty topology so the caller-route lookup stream actually evaluates its predicate.
    CamelRoute route = new CamelRoute("routeA", "direct:start",
        List.of(new CamelRouteOutput("epX", "To[mock:out]", null, "ToDefinition", null)),
        false, null);
    when(routeContextService.getCamelRoutes()).thenReturn(List.of(route));

    // A previously created message for this exchange so the back-fill forEach runs.
    Message previous = new Message(exchange.getExchangeId(), MessageEventType.CREATED, null, null,
        null, "direct:start", null, MessageType.REQUEST, null);
