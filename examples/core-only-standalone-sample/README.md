# core-only-standalone-sample

Minimal camel-main (plain Camel) application that uses **`camelbee-standalone-core` as a plain dependency** — README
...
It is **not** a feature demo. [`allcomponent-standalone-sample`](../allcomponent-standalone-sample) is that.
...
- **The published version floor is real.** It pins Camel 4.12.0, the oldest combination the
...
```bash
mvn -pl examples/core-only-standalone-sample -am install   # from the repo root
mvn -f examples/core-only-standalone-sample/pom.xml compile exec:java -Dexec.mainClass=io.camelbee.coreonly.standalone.Application
```

Then open <http://localhost:8081/camelbee>.
