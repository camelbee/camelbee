# core-only-quarkus-sample

Minimal Quarkus application that uses **`camelbee-quarkus-core` as a plain dependency** — README
...
It is **not** a feature demo. [`allcomponent-quarkus-sample`](../allcomponent-quarkus-sample) is that.
...
- **The published version floor is real.** It pins Quarkus 3.15.4 LTS + Camel Quarkus 3.15.3, the oldest combination the
...
```bash
mvn -pl examples/core-only-quarkus-sample -am install   # from the repo root
mvn -f examples/core-only-quarkus-sample/pom.xml quarkus:dev
```
