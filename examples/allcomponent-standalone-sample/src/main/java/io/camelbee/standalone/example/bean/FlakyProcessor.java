...
package io.camelbee.standalone.example.bean;

import org.apache.camel.Exchange;

/**
 * Simulates a transient failure so the demo produces redelivery traffic without any external infra.
 *
 * <p>The attempt counter lives on the exchange, which survives redelivery, so every top-level
 * exchange fails exactly {@value #FAILING_ATTEMPTS} times and then succeeds - deterministic enough
 * to assert on from an integration test.
 */
